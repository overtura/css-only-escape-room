...
## Product Intent
HTML/CSS만으로 진행되는 한국어 퍼즐형 escape room이다. 사용자는 checkbox 상태와 `:has()` 선택자만으로 전등, 열쇠, 암호, 출구 상태가 변하는 흐름을 경험한다.
...
- 어두운 방 배경과 warm key light를 사용한다.
- amber는 진행 가능/성공, cyan은 발견/활성 단서, red는 실패/위험에만 사용한다.
...
- runtime JavaScript와 canvas/WebGL 게임 엔진은 쓰지 않는다.
